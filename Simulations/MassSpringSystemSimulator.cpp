...
void MassSpringSystemSimulator::initUI(DrawingUtilitiesClass* DUC)
{
	...
	case 3:
		...
		method = TwDefineEnumFromString("Gravity", "True,False");
		TwAddVarRW(DUC->g_pTweakBar, "Gravity", method, &m_iGravity, "");
		break;
	default:break;
	}
}
...
void MassSpringSystemSimulator::notifyCaseChanged(int testCase)
{
	...
	case 0:
		...
		// reset
		m_massPoints.at(0)->m_position = Vec3(0, 0, 0);
		m_massPoints.at(0)->m_velocity = Vec3(-1, 0, 0);
		m_massPoints.at(1)->m_position = Vec3(0, 2, 0);
		m_massPoints.at(1)->m_velocity = Vec3(1, 0, 0);
	...
	switch (m_iIntegrator)
	{
		case EULER:
			...
			{	
				...
				cout << "Point index: " << i << "\n";
				cout << "Position: " << masspoint->m_position << "\n";
				cout << "Velocity: " << masspoint->m_velocity << "\n";
		...
		case MIDPOINT:
			...
			{
				MassPoint* masspoint = m_massPoints.at(i);
				Vec3 midPosition = integratePosition(masspoint->m_position, masspoint->m_velocity, timeStep / 2.0);
				Vec3 midVelocity = integrateVelocity(masspoint->m_velocity, masspoint->m_internalForce, timeStep / 2.0);
				oldPositions.push_back(masspoint->m_position);
				masspoint->m_position = midPosition;
				midVelocities.push_back(midVelocity);

				masspoint->m_internalForce = Vec3(); // reset

				// Vec3 midInternalForce = applyInternalForce(midPosition, midVelocity, masspoint->m_attachedSprings);
				// masspoint->m_position = integratePosition(masspoint->m_position, midVelocity, timeStep);
				// masspoint->m_velocity = integrateVelocity(masspoint->m_velocity, midInternalForce, timeStep);
			}
			...
			for (int i = 0; i < getNumberOfMassPoints();i++) {
				...
				masspoint->m_position = integratePosition(oldPositions.at(i), midVelocities.at(i), timeStep);
				masspoint->m_velocity = integrateVelocity(masspoint->m_velocity, masspoint->m_internalForce, timeStep);
...
void MassSpringSystemSimulator::applyExternalForce(Vec3 force)
{
	for (int i = 0; i < getNumberOfMassPoints();i++) {
		MassPoint* masspoint = m_massPoints.at(i);
		masspoint->m_internalForce += force;
	}
}
...
{		
	...
	spring->m_fCurrentLength = sqrt(point1->m_position.squaredDistanceTo(point2->m_position));
	float springTerm = -m_fStiffness * (spring->m_fCurrentLength - spring->m_fInitialLength) / spring->m_fCurrentLength;
